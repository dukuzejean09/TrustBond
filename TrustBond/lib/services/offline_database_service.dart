...
class OfflineDatabaseService {
  ...
  Future<Map<String, int>> getQueueStats() async {
    ...
    final reportsResult = await db.rawQuery('''
      SELECT 
        COUNT(CASE WHEN status = 'queued' THEN 1 END) as queued_reports,
    ...
    final evidenceResult = await db.rawQuery('''
      SELECT 
        COUNT(CASE WHEN status = 'pending' THEN 1 END) as pending_evidence,
        COUNT(CASE WHEN status = 'error' THEN 1 END) as failed_evidence,
        COUNT(CASE WHEN status = 'completed' THEN 1 END) as completed_evidence
      FROM evidence_queue
    ''');
