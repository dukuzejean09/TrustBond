...
function App() {
  ...
  useEffect(() => {
    ...
    const path = window.location.pathname || "/";
    if (user) {
      setTimeout(() => setCurrentScreen(pathToScreen(path)), 0);
    } else {
      setTimeout(() => setAuthScreen(pathToAuth(path)), 0);
    }
    ...
    const loadCounts = async () => {
      try {
        ...
        setSidebarCounts({
          ...
          cases: caseStats
            ? (caseStats?.open ?? 0) + (caseStats?.in_progress ?? 0)
            : (dash?.open_cases ?? 0),
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case "case-management":
        return <CaseManagement goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "hotspots":
        return <Hotspots goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "hotspot-details":
        return <HotspotDetails hotspotId={selectedHotspotId} wsRefreshKey={wsRefreshKey} />;
      case "safety-map":
        return <SafetyMap goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "device-trust":
        return <DeviceTrust goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "users":
        return (
          <Users
            goToScreen={goToScreen}
      ...
      case "audit-log":
        return <AuditLog goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "system-config":
        return <SystemConfig goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      case "change-password":
      ...
      case "active-sessions":
        return <ActiveSessions goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
      default:
        return <Dashboard goToScreen={goToScreen} openModal={openModal} wsRefreshKey={wsRefreshKey} />;
    }
  };
